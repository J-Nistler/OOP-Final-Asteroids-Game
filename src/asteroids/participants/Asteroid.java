 ...
 */
public class Asteroid extends Participant implements ShipDestroyer, ShipBulletDestroyer
{
    ...
    public void collidedWith (Participant p)
    {
        if (p instanceof AsteroidDestroyer)
        {
            ...
            if (this.getSize() == 2)
            {
                Participant.getSounds().playBangLargeNoise();
            }
            if (this.getSize() == 1)
            {
                Participant.getSounds().playBangMediumNoise();
            }
            if (this.getSize() == 0)
            {
                Participant.getSounds().playBangSmallNoise();
            }
            
            // Creates debris where the asteroids once were
            
            controller.addParticipant(new Parts(this.getX(), this.getY(), 2));
            controller.addParticipant(new Parts(this.getX(), this.getY(), 2));
            controller.addParticipant(new Parts(this.getX(), this.getY(), 2));
            controller.addParticipant(new Parts(this.getX(), this.getY(), 2));
            ...
            if (size == 1)
            {
                // Generates a random speed for the new asteroid, capping it at it's maximum value
                double speed = ThreadLocalRandom.current().nextDouble(1.0, MAXIMUM_MEDIUM_ASTEROID_SPEED);
                
                this.controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, this.getX(), this.getY(),
                        speed, this.controller));
                this.controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, this.getX(), this.getY(),
                        speed, this.controller));
            }
            ...
            {
                // Generates a random speed for the new asteroid, capping it at it's maximum value
                double speed = ThreadLocalRandom.current().nextDouble(1.0, MAXIMUM_SMALL_ASTEROID_SPEED);
